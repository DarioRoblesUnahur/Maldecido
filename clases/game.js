...
class Game {
    ...
    _enemigoDispara(x, y, tx, ty, dano) {
        ...
        this.proyEnemigos.push({
            g,
            vx: Math.cos(ang) * c.velocidad,
            vy: Math.sin(ang) * c.velocidad,
            vida: c.vida,
            dano
        });
    }
    ...
    _abrirNivel() {
        ...
        this._mostrarSelector('⬆ ¡Subiste de nivel!', `Nivel ${this.jugador.nivel} — ${this.jugador.nombreTrance}`,
            opciones, 'panel-nivel', 'nivel-grid', () => {
                ...
                else if (this.jugador._totemOroPendiente) {
    ...
    _opcionTesoro() {
        return {
            ...
            aplicar: () => {
                this.jugador.vidaMax += CONFIG.jugador.reliquiaVida;
                this.jugador.curar(this.jugador.vidaMax);
            },
        };
    }
    ...
    _actualizarConsumibles(delta) {
        for (let i = this.consumibles.length - 1; i >= 0; i--) {
            ...
            if (c.update(delta, this.jugador)) {
                c.destroy();
